extends CharacterState

var hit = false
var tp_pos_x = 0
var tp_pos_y = 0
var tp_vel_x = "0"
var tp_vel_y = "0"

const RAPID_SLASHES_SCENE = preload("res://Maestro/characters/Maestro/projectiles/RapidSlashes.tscn")

const EXTRA_FRAME_PER_PIXEL = "0.05"
const MIN_LAG_DIST = "200"
const DIST = 16

const max_lag_frames = 8
var lag_frames = 0
var dir = 1

export  var teleport = true

func _frame_0():
	host.z_index = - 2
	lag_frames = 0
	hit = false

func _frame_8():
	if teleport:
		var opp_pos = host.opponent.get_pos()
		var opp_pos_local = host.obj_local_center(host.opponent)
		var distance = fixed.vec_len(str(opp_pos_local.x), str(opp_pos_local.y))
		if fixed.gt(distance, MIN_LAG_DIST):
			lag_frames = Utils.int_min(fixed.round(fixed.mul(EXTRA_FRAME_PER_PIXEL, fixed.sub(distance, MIN_LAG_DIST))), max_lag_frames)
		host.colliding_with_opponent = false
		var opp_vel = host.opponent.get_vel()
		tp_pos_x = opp_pos.x + DIST * host.get_opponent_dir()
		dir = host.get_opponent_dir()
		tp_pos_y = opp_pos.y
		tp_vel_x = opp_vel.x
		tp_vel_y = opp_vel.y

func _frame_12():
	if teleport:
		host.set_vel(tp_vel_x, tp_vel_y)
		host.set_pos(tp_pos_x, tp_pos_y)

		var opp_pos = host.opponent.get_pos()
		host.spawn_object(RAPID_SLASHES_SCENE, opp_pos.x, opp_pos.y - 12, true, null, false)

func _frame_13():
	if teleport:
		host.set_facing(dir * - 1)

		var opp_pos = host.opponent.get_pos()
		host.spawn_object(RAPID_SLASHES_SCENE, opp_pos.x, opp_pos.y - 12, true, null, false)


func _frame_14():
	if teleport:
		var opp_pos = host.opponent.get_pos()
		host.spawn_object(RAPID_SLASHES_SCENE, opp_pos.x, opp_pos.y - 12, true, null, false)

func _tick():
	host.apply_fric()
	host.apply_forces()
	if not (teleport and hit):
		host.apply_grav()
	if current_tick >= 8:
		if lag_frames > 0:
			lag_frames -= 1
			current_tick = 8
	elif current_tick > 30:
		host.apply_grav()
		

func _on_hit_something(obj, hitbox):
	hit = true
	if host.actions == 1:
		host.unlock_achievement("ACH_TELEPORTS_BEHIND_YOU", true)

	._on_hit_something(obj, hitbox)




